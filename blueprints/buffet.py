...
def build_buffet_prep_sheet(cur, event_id, unit_system='imperial'):
    ...
    if station_pull_specs:
        ...
        for station_name in sorted(station_pull_specs):
            ...
            detail_sheets.sort(
                key=lambda item: (
                    {'linked': 0, 'subrecipe': 1}.get(item.get('sheet_type_key'), 9),
    ...
    if prep_data['prep_station_groups']:
        ...
        for station_index, station_group in enumerate(prep_data['prep_station_groups']):
            ...
            if station_group.get('direct_lines'):
                        ...
                        [
                            ...
                            p(
                                ' / '.join(
                                    part
                                    for part in [
                                        row.get('description') or 'Pull to station spec.',
                                        row.get('vessel') or '',
                                        row.get('foh_talking_points') or '',
                                    ]
                                    if part
                                )
                            ),
                ...
                direct_table = Table(
                    direct_rows,
                    colWidths=[1.45 * inch, 1.75 * inch, 1.05 * inch, 5.45 * inch],
                    repeatRows=1,
                )
            ...
            if station_group.get('detail_sheets'):
                ...
                for detail_index, sheet in enumerate(station_group['detail_sheets']):
                    ...
                    component_table = Table(
                        component_rows,
                        colWidths=[5.4 * inch, 1.05 * inch, 0.9 * inch, 2.05 * inch],
                        repeatRows=1,
                    )
